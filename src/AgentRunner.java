...
public class AgentRunner implements ActionListener, ChangeListener
{
  ...
  public AgentRunner()
  {
    ...
    // Will need to make a more elegant solution later so that we don't need to load agents one by
    // one
  ...
  public void loadNewRoom()
  {
    ...
    startingDir = (startingDir.exists() && startingDir.isDirectory()) ? startingDir : new File(".");
  ...
  protected void refreshRoom()
  {
    try
    {
      ...
      /*
       * FIX: setting the roomGUI's preferredSize to that of its parent pane works well on initial
       * load, but still doesn't handle window resizes.
       */
    ...
    {
      JOptionPane.showMessageDialog(window, "Could not open file: " + ioe.getMessage(),
   ...
   * 
   * @param newMessage    The Message to add to the message queue.
   * 
   * @return A boolean that is true if the Message was added correctly,
   *                           false otherwise.
   *                           
   */
  ...
  {
    return this.messages.offer(newMessage);
  }
  
  /**
   * Returns the first Message in the message queue.
   * 
   * @return The Message at the head of the message queue.
   *  
   */
